...
var RemoteGridController = AbstractGridController.extend({
    ...
    init: function ($rootScope, $scope, $state, Notifications, datacontext, permissions) {
        ...
        this._super($rootScope, $scope, Notifications, permissions);
    },
    ...
    _queryDatabase: function () {
        ...
        this._datacontext.executeQuery(this._getQuery(), this._querySucceeded.bind(this), this._queryFailed.bind(this));
    },
    ...
    _onQueryParamsChanged: function () {
        if (!_.isNumber(this.$scope.queryParams.take)) {
            this.$scope.queryParams.take = this._queryParams.take;
        } else if (this._datacontext.hasNewItems()) {
            toastr.warning('Save new items before updating page', 'Warning!');
        } else if (this._isReady) {
            this._queryParams.take = this.$scope.queryParams.take;
            this._queryDatabase();
        }
    },
...
RemoteGridController.$inject = ['$rootScope', '$scope', '$state', 'Notifications', 'datacontext', 'permissions'];